---
description: Find aube settings, defaults, configuration sources, and managed policies.
editLink: false
---
...
Look up a setting to see its default, accepted values, and supported configuration sources. Start with the [configuration guide](/package-manager/configuration) if you are choosing where to put a value.

Use `aube config find <words>` to search from your terminal, or `aube config explain <key>` to inspect a setting. A listed source means the setting accepts that input; not every setting has a CLI flag.

## Browse by category

- [Dependency Resolution](#dependency-resolution)
- [Dependency Hoisting](#dependency-hoisting)
...
- [Node.js](#node-js)
- [Other](#other)
- [aube-specific](#aube-specific)

## Summary

<details>
<summary>Expand the complete settings index</summary>
...
This page is generated from [`settings.toml`](https://github.com/aubepkg/aube/blob/main/crates/aube-settings/settings.toml). Edit the registry and run `cargo run -p aube-settings --bin generate-settings-docs` to regenerate it.
